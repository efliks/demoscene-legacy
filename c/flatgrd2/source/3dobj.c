...
OBJECT3D* load_object3d(char* filename)
{
    ...
    if ((obj->points = (POINT3D*)malloc(obj->n_points * sizeof(POINT3D))) == NULL || (obj->rotated_points = (POINT3D*)malloc(obj->n_points * sizeof(POINT3D))) == NULL || (obj->translated_points = (POINT2D*)malloc(obj->n_points * sizeof(POINT2D))) == NULL || (obj->point_normals = (VECTOR3D*)malloc(obj->n_points * sizeof(VECTOR3D))) == NULL || (obj->rotated_point_normals = (VECTOR3D*)malloc(obj->n_points * sizeof(VECTOR3D))) == NULL || (obj->faces = (FACE*)malloc(obj->n_faces * sizeof(FACE))) == NULL || (obj->face_normals = (VECTOR3D*)malloc(obj->n_faces * sizeof(VECTOR3D))) == NULL || (obj->face_depth = (short int*)malloc(obj->n_faces * sizeof(int))) == NULL || (obj->order_table = (short int*)malloc(obj->n_faces * sizeof(int))) == NULL)
...
void unload_object3d(OBJECT3D* obj)
{
    ...
    if (obj->translated_points)
        free(obj->translated_points);
    if (obj->point_normals)
        free(obj->point_normals);
    if (obj->rotated_point_normals)
        free(obj->rotated_point_normals);
    if (obj->faces)
        free(obj->faces);
    if (obj->face_normals)
        free(obj->face_normals);
    if (obj->face_depth)
        free(obj->face_depth);
    if (obj->order_table)
        free(obj->order_table);
...
void init_point_normals(OBJECT3D* obj)
{
    ...
    for (i = 0; i < obj->n_points; i++) {
        ...
        for (j = 0; j < obj->n_faces; j++) {
            if (obj->faces[j].v1 == i || obj->faces[j].v2 == i || obj->faces[j].v3 == i) {
...
void depth_sorting(OBJECT3D* obj, short int left, short int right)
{
    ...
    while (i < j) {
        while (obj->face_depth[i] < element)
            i++;
        while (obj->face_depth[j] > element)
            j--;
    ...
    if (j > left)
        depth_sorting(obj, left, j);
    if (i < right)
        depth_sorting(obj, i, right);
}
